...
export interface MapaPageClientProps {
  initiatives: Initiative[];
  /** When set, renders a DataUnavailableMessage in place of the search/filter bar and the map. */
  state?: "empty" | "error";
}

// ── Page ───────────────────────────────────────────────────────────────────

export function MapaPageClient({ initiatives, state }: MapaPageClientProps) {
    ...
    <div className="relative flex flex-col h-screen w-full overflow-hidden bg-white">
      ...
      {state ? (
        <div className="relative z-10 flex-1 min-h-0 flex items-center justify-center">
          <DataUnavailableMessage variant={state} title="Mapa no disponible" description="No fue posible cargar las iniciativas en este momento." />
        </div>
      ) : (
        <>
          {/* Search + filter bar */}
