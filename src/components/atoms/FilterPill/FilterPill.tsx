...
export type FilterPillVariant = "active" | "default" | "teal" | "tealOutline" | "purple" | "navy";
...
const variantMap: Record<FilterPillVariant, string> = {
  ...
  tealOutline:  "bg-transparent text-primary border border-primary hover:bg-primary/10",
...
export function FilterPill({ variant = "default", children, className, disabled, ...props }: FilterPillProps) {
  return (
    <button
      type="button"
      aria-pressed={variant === "active" || variant === "teal" || variant === "tealOutline" || variant === "purple" || variant === "navy"}
      disabled={disabled}
      className={cn(
