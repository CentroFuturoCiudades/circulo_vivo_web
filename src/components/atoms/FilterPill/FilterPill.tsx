...
export type FilterPillVariant = "active" | "default" | "teal" | "purple";
...
export function FilterPill({ variant = "default", children, className, disabled, ...props }: FilterPillProps) {
  return (
    <button
      type="button"
      aria-pressed={variant === "active" || variant === "teal" || variant === "purple"}
      disabled={disabled}
      className={cn(
        "inline-flex items-center justify-center rounded-full px-4 h-7 font-sans text-[10px] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary select-none whitespace-nowrap",
        variantMap[variant],
